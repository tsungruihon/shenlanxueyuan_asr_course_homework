...
BaseFloat LatticeFasterDecoderTpl<FST, Token>::ProcessEmitting(
  ...
  size_t tok_cnt;
  // Beam prune参数获取，包括cur_cutoff, adaptive_beam, best_elem, 后两者用来 确定next_cutoff
  // 主要是两个条件，默认是best_weight + config_.beam
  // 同时用config_.max_active, config_.min_active 做了加强
  // 希望state数目在[config_.min_active, config_.max_active]之间
  // 通过GetCutoff()获取histogram的剪枝阈值
  // 该函数内部用max_active, min_active来控制historgram pruning保留的token数。
  // 并且通过Beam Pruning通过GetCutoff()获取了best_token
  ...
  // BaseFloat cur_cutoff = GetCutoff(final_toks, &tok_cnt, &adaptive_beam, &best_elem);
  // 输入final_toks, HashList对应的list， toks_.Clear()操作后的头结点指向
  // 输出cur_cutoff, 返回值，用于现在时刻Beam Prune
  // 输出adaptive_beam, best_elem, 得到next_cutoff，用于下一时刻Beam Prune
  // 输出tok_cnt， 用于重置HashList toks_大小，足够大，减少内存分配时间
  ...
  // next_cutoff用于下一时刻state的beam prune。 等于现在时刻最优state到下一时刻对应所有state中最优的tot_cost
  // cost_offset 只是为了计算方面的考虑，相当于同时减了一个最小数。
  // 通过对best_token进行传递，配合adaptive_beam（在GetCutoff中对beam调整）
  // 获得beam pruning的阈值next_cutoff
  if (best_elem) {
  ...
  for (Elem *e = final_toks, *e_tail; e != NULL; e = e_tail) {
    ...
    StateId state = e->key;
    Token *tok = e->val;
    // 现在时刻beam prune，tot_cost控制在cur_cutoff阈值以内
    // cur_cutoff=现在时刻最优state tot_cost + beam
    // 通过比较token的alpha值和cur_cutoff（即histogram pruning阈值）来判断并完成historgram pruning
    if (tok->tot_cost <= cur_cutoff) {
      // 内层循环，遍历现在时刻某个state的所有跳转
      for (fst::ArcIterator<FST> aiter(*fst_, state);
           !aiter.Done();
           aiter.Next()) {
        const Arc &arc = aiter.Value();
        ...
        if (arc.ilabel != 0) {  // propagate..
          ...
          // next_cutoff, 初始值为：现在时刻最优state到下一时刻所有state中最优cost+adaptive_bream
          // 当更好的阈值出现时，动态更新beam pruning阈值
          else if (tot_cost + adaptive_beam < next_cutoff)
            next_cutoff = tot_cost + adaptive_beam; // prune by best current token
          // Note: the frame indexes into active_toks_ are one-based,
          // hence the + 1.
          // 扩展下一时刻token，存取在toks_中
          // 这一帧的ProcessNonemitting就是在toks_对应的list中循环
          // 所以说ProcessNonemiting循环的是下一时刻的state以及下一时刻state的扩展跳转。
  ...
  while (!queue_.empty()) {
    ...
    for (fst::ArcIterator<FST> aiter(*fst_, state);
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        ...
        // 进行beam pruning，其中cutoff值来自于ProcessEmitting()中获得的next_cutoff
        // 或者是初始时为beam值
        if (tot_cost < cutoff) {
          bool changed;
